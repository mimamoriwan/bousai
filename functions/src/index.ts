...
export const aggregateWalkActionCreated = onDocumentCreated(
  ...
  async (event) => {
    ...
    if (data) {
      const processed = await updateWalkHeatmap(adminDb, data, 1, event.id);
      if (!processed) {
        logger.warn("Skipping invalid walk action heatmap event", { eventId: event.id });
      }
    }
  }
);
